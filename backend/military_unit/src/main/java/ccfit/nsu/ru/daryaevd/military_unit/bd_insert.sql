...
INSERT INTO soldier_table (first_name, last_name, date_of_birth, military_card, date_of_issue_of_military_card,
                           mas_id, soldier_type_id, subdivision_id, is_commander, commander_id)
VALUES
    ('Amanda', 'Davis', '1993-07-12', 'MNO345', '2020-06-20', 3,
     (SELECT id FROM soldier_type WHERE type_rank = 8 LIMIT 1),
     (SELECT s.id FROM subdivision_table s JOIN subdivision_type st ON s.type_of_subdivision_id = st.id WHERE st.subdivision_rank = 7 LIMIT 1), false, null),

    ('Christopher', 'Brown', '1985-09-05', 'JKL012', '2020-05-10', 2,
     (SELECT id FROM soldier_type WHERE type_rank = 6 LIMIT 1),
     (SELECT s.id FROM subdivision_table s JOIN subdivision_type st ON s.type_of_subdivision_id = st.id WHERE st.subdivision_rank = 6 LIMIT 1), false, 1),

    ('Emily', 'Williams', '1992-04-25', 'GHI789', '2020-04-30', 3,
     (SELECT id FROM soldier_type WHERE type_rank = 4 LIMIT 1),
     (SELECT s.id FROM subdivision_table s JOIN subdivision_type st ON s.type_of_subdivision_id = st.id WHERE st.subdivision_rank = 5 LIMIT 1), false, 2),

    ('Michael', 'Johnson', '1988-11-10', 'DEF456', '2020-03-20', 1,
     (SELECT id FROM soldier_type WHERE type_rank = 3 LIMIT 1),
     (SELECT s.id FROM subdivision_table s JOIN subdivision_type st ON s.type_of_subdivision_id = st.id WHERE st.subdivision_rank = 4 LIMIT 1), false, 3),

    ('Potap', 'Arturov', '1983-04-22', 'sdgrge', '2018-12-20', 1,
     (SELECT id FROM soldier_type WHERE type_rank = 7 LIMIT 1),
     (SELECT s.id FROM subdivision_table s JOIN subdivision_type st ON s.type_of_subdivision_id = st.id WHERE st.subdivision_rank = 3 LIMIT 1), false, 4),
    ...
    ('John', 'Doe', '1990-05-15', 'ABC123', '2020-01-01', 1,
     ...
     (SELECT s.id FROM subdivision_table s JOIN subdivision_type st ON s.type_of_subdivision_id = st.id WHERE st.subdivision_rank = 2 LIMIT 1), false, 6),

    ('Kirill', 'Kirillov', '1980-03-22', 'YZAdgfd567', '2010-10-20', 1,
     (SELECT id FROM soldier_type WHERE type_rank = 7 LIMIT 1),
     (SELECT s.id FROM subdivision_table s JOIN subdivision_type st ON s.type_of_subdivision_id = st.id WHERE st.subdivision_rank = 1 LIMIT 1), false, 7),

    ('Seva', 'Nosov', '1980-05-15', 'hahaha123', '2010-01-01', 3,
     (SELECT id FROM soldier_type WHERE type_rank = 5 LIMIT 1),
     (SELECT s.id FROM subdivision_table s JOIN subdivision_type st ON s.type_of_subdivision_id = st.id WHERE st.subdivision_rank = 0 LIMIT 1), false, 8);
